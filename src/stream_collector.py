...
def stream_camera_worker(
    url, id, width, height, fps, display_queue, producer, topic, display=False
):
    """
    ...
    while cap.isOpened():
        ...
        producer.produce(
            topic, key=id, value=json.dumps(message), callback=delivery_report
        )
        producer.poll(0)
...
def start_camera_stream(
    urls, ids, width, height, fps, producer, topic, display=False
):
    """
...
if __name__ == "__main__":
    ...
    LOCAL_CONFIG_PATH = (
        args.config if args.config else "../config/collector/file_cam_local.yaml"
    )
    ...
    start_camera_stream(
        URLS, IDS, WIDTH, HEIGHT, FPS, producer, topic, DISPLAY
    )
    producer.flush()