...
cap = cv2.VideoCapture("./resources/walk.mp4")
...
while True:
    ...
    img_blob = cv2.dnn.blobFromImage(
        img_to_detect, 0.003922, (416, 416), swapRB=True, crop=False)
    ...
    yolo_model = cv2.dnn.readNetFromDarknet(
        'models/yolov3.cfg', 'models/yolov3.weights')
    yolo_model.setPreferableBackend(cv2.dnn.DNN_BACKEND_OPENCV)
    yolo_model.setPreferableTarget(cv2.dnn.DNN_TARGET_OPENCL)
    ...
    for max_valueid in max_value_ids:
        ...
        predicted_class_label = "{}: {:.2f}%".format(
            predicted_class_label, prediction_confidence * 100)
