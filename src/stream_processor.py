...
# sys.path.append("/opt/spark/apps/src")
...
from config.processor.schemas import (
    video_stream_schema,
    movement_by_frame_output_schema,
)
...
class VideoStreamProcessor:
    ...
    def __init__(self, config_path: str) -> None:
        ...
        self.spark = (
            SparkSession.builder.appName("VideoStreamProcessor")
            .config(
                "spark.jars.packages",
    ...
    def read_from_kafka(self):
        return (
            ...
            .option(
                ...
                self.config["kafka"]["max_batch_size"],
            )
            .option(
            ...
            .select(
                from_json(col("message"), video_stream_schema).alias("json")
            )
            .select("json.*")
        )
    ...
    def process_data(self):
        uf_fun = self.define_function(movement_by_frame)

        df = self.read_from_kafka()

        processed_df = df.groupBy("cameraId").agg(
            uf_fun(collect_list(struct(col("timestamp"), col("data")))).alias(
                "movement_by_frame"
            )
        )
        # Process data
...
if __name__ == "__main__":
    processor = VideoStreamProcessor("/opt/spark/apps/config/processor/variables.yaml")
